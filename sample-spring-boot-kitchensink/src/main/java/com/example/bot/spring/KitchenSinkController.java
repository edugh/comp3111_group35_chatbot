...
public class KitchenSinkController {
	...
	private boolean matchingQA(String question, String text) {
		List<char[]> bigram1 = bigram(question);
		List<char[]> bigram2 = bigram(text);
		return (stringSimilarityScore(bigram1, bigram2) > 0.5);
	...
	private List<Message> tryHandleFAQ(String text, Source source) {
		...
		for (FAQ faq: faqs) {
			if (matchingQA(faq.question, text)) {
	...
	private List<Message> tryHandleTourSearch(String text, Source source) {
		...
		if (matchingQA("Which tours are available", text)) {
